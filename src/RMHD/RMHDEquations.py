...
def m_x(D, h, gamma, B_x, B_y, B_z, v_x, v_y, v_z):
    return (D * h * gamma + (B_x ** 2 + B_y ** 2 + B_z ** 2)) * v_x - (v_x * B_x + v_y * B_y + v_z * B_z) * B_x

def m_y(D, h, gamma, B_x, B_y, B_z, v_x, v_y, v_z):
    return (D * h * gamma + (B_x ** 2 + B_y ** 2 + B_z ** 2)) * v_y - (v_x * B_x + v_y * B_y + v_z * B_z) * B_y

def m_z(D, h, gamma, B_x, B_y, B_z, v_x, v_y, v_z):
    return (D * h * gamma + (B_x ** 2 + B_y ** 2 + B_z ** 2)) * v_z - (v_x * B_x + v_y * B_y + v_z * B_z) * B_z

def E(D, h, gamma, B_x, B_y, B_z, v_x, v_y, v_z, p):
    return D * h * gamma - p + (B_x ** 2 + B_y ** 2 + B_z ** 2)/2 + ((v_x ** 2 + v_y ** 2 + v_z ** 2) * (B_x ** 2 + B_y ** 2 + B_z ** 2) - (v_x * B_x + v_y * B_y + v_z * B_z) ** 2)/2
...
def energy(D, h, gamma, B, v, p):
    ...
    return D * h * gamma - p + magnetic_energy_term/2 + 0.5 * (kinetic_energy_term * magnetic_energy_term - torch.dot(v, B) ** 2)
